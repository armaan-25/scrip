# SpecSpend Learning Notes

## Concepts demonstrated

- A bearer credential proves possession, so it must be short-lived, scoped,
  revocable, and never logged. SpecSpend hashes credentials with Node's
...
- `max_tokens` provides an output-cost ceiling. SpecSpend combines it with a
...
- `js-yaml` loads human-readable Ramp budget mappings from `specspend.yaml`.
